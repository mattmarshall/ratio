...
export const BOOK_TEMPLATES: readonly BookTemplate[] = [
  {
    ...
    blurb: "Cash and bank, living expenses, income, named loans, and a credit-card liability. A household budget is a configuration total, not a second ledger. `brokerage-statement` / `brokerage-positions` ingest a custodian or broker CSV onto Investments as transfers — an unmatched instrument pends, unidentified or foreign-currency holdings refuse the live recon, never a silent 0. Lot relief stays unset until `[personal] lot_relief = true` elects the wash / MinTax / SpecID / average-cost engines already on the book — not a silent lot, not fund ABOR chrome.",
  },
  {
    kind: "INVESTMENT",
  ...
  },
  {
    kind: "PROJECT",
    label: "Project",
    blurb: "Cash, work in progress, project costs by work package, funding, revenue, progress billings, retainage, approved change orders, and awarded commitments. Budget vs actual and billed vs earned are two figures, not one URL. A change order adjusts the revised contract without rewriting the original baseline. `/budget` posts or ingests an approved change order or award with phase keys; facts stay unset until posted. `/billing` posts a cash application (`collect_receivable`); collected stays unset until billed and receivable can support the cut — not a fake zero. `project-invoices` ingests a job-cost / AP / progress-bill statement into the journal; an unidentified vendor pends and a retainage or WIP kind is refused — ingest does not invent a holdback. Remaining to spend, remaining to bill, and collections vs billed stay unset until the journal can support them. The budget page does not forecast.",
  },
  {
    kind: "OPERATING",
...
export const INGEST_TEMPLATE_KIND: Readonly<Record<string, Exclude<BookKind, "UNSPECIFIED">>> = {
  ...
  "brokerage-statement": "PERSONAL",
  "brokerage-positions": "PERSONAL",
