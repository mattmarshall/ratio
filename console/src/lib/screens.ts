...
//
// ⭐ KIND SELECTS THE LIST. An investment book leads with capital / partners
// and still offers the fund ABOR screens (exceptions, positions, NAV) — issue
// #70. Personal and project stay on the operations surface here; their own
// figures are #65 / #66.
...
export function screensFor(kind: BookKind): readonly Screen[] {
  return kind === "INVESTMENT" ? INVESTMENT_SCREENS : FUND_SCREENS;
}

/** Where a newly opened book of record lands. */
export function defaultScreen(kind: BookKind): string {
  return kind === "INVESTMENT" ? "capital" : "breaks";
}
...
export function ticketsFor(_kind: BookKind): readonly Ticket[] {
  return FUND_TICKETS;
}
