...
// / NAV would be a fake label on fund-ops screens — issue #83 (budget) sits
// beside #65 (sheet/P&L). Project budget/WIP is #66; this file leaves Project
// on the fund list until that lands. The agreement screens stay shared: a
// rule set is the same document whichever chart it posted.
...
/**
 * Household figures. Budget vs actual against `chart_for(Personal)`, not ABOR.
 *
 * Trial balance stays: it is the conservation view of the same accounts, and
 * a figure that could not be checked against it would be a picture. Sheet and
 * period P&L remain #65.
 */
...
export function defaultScreen(kind: BookKind): string {
  return kind === "PERSONAL" ? "budget" : "breaks";
}
...
const PERSONAL_TICKETS: readonly Ticket[] = [
  { segment: "record", label: "Record an event", keywords: "record,event,rule,apply,spend" },
  {
    segment: "ingest",
    label: "Ingest a delivery",
    keywords: "ingest,delivery,file,statement,admit",
  },
];
