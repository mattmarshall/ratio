...
// / NAV would be a fake label on fund-ops screens. Progress billing, retainage
// and cost-by-phase live here; budget vs actual / WIP capitalization are #66
// / PR #80 and use `/budget` and `/wip` — this issue does not share those
// paths. Personal household figures are #65. The agreement screens stay
// shared: a rule set is the same document whichever chart it posted.
...
/**
 * Project figures. Progress billing and retainage, not ABOR.
 *
...
export function defaultScreen(kind: BookKind): string {
  return kind === "PROJECT" ? "billing" : "breaks";
}
...
const PROJECT_TICKETS: readonly Ticket[] = [
  {
    ...
    keywords: "record,event,rule,apply,bill,retainage,cost",
  },
  {
    segment: "ingest",
    label: "Ingest a delivery",
