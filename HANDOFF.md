...
Deploy smoke already retried that body; the console painted the
first 503 as a dead-end. The console now retries a hydrating 503
the way smoke does. This
...
(`connect/tax-pack/`, #166) cites that same rule. Its disposal resource replays
the selected view and returns the exact relieved lots, journal prefix,
configuration digest, historically pinned holding-period threshold, and later
wash write. A lot break refuses the export, and `relieved_lots.csv` preserves
the exact lot rows behind the aggregate basis. The dedicated WorkOS application
is registered, and a post-deploy live export against the deterministic
`personal-tax-walkthrough` seed proved the wash and missing-date rows with
their exact cites. IRS e-file and a CPA portal remain refused. The Project AIA
...
cites partner capital, commitments / undrawn, capital notices,
GetBook `partner_cut` / specials / facts / fee receivable / trial
balance, and NavStrike / period roll-forward already on the book;
a missing cite stays unset — never a silent 1/N of book NAV, a
callable-zero commitment, a NAV 0.00, or an empty-digest-as-success.
`as_html` is a read-only cite-backed page in that tree. Read-only:
no `journals:post`. No kernel portal, no LP directory, no document
vault, no payment initiation. That does not close #161 (a hosted
live LP walk-through and drip elections remain). This app's
Connect grant is proven. #22 stays open for other Connect apps.
The fund-ops-alerts Connect app (`connect/fund-ops-alerts/`, #162)
cites unexplained breaks, unpriced marks, and NAV gate blocking
reasons already on the book (`nav_gate` / the same `blocking_at`
fold the console badge reads); a missing cite stays unset — never
a silent empty break list that looks reconciled, an all-clear
gate, a NAV 0.00, or an empty-digest-as-success. Read-only: no
`journals:post`. No kernel notifier, no chatbot, no HTML alert UI
inside `ratio watch`. Slack / email / PagerDuty stay leftover.
...
`connect/personal_activation.py` (#324) binds that verified subject and selected
...
| `crates/ratio-project` | the read model, the lot book, the relief engine — one pass, N view folds, each with a monotonic cut on the journal's own clock and a band bounded by the settlement lag. ⚠ every memory figure in this file is a ONE-VIEW figure; each view carries its own lot book. Verified journal-prefix checkpoints (`checkpoint`, #310): content-addressed blob + atomic HEAD; pin is prefix + `prefix_digest`; corrupt/mismatch falls back to full replay; crova evaluated and deferred |
...
| `connect/bank-feed/` | First-party Connect app for Personal bank feeds ([#165](https://github.com/mattmarshall/ratio/issues/165), provider slices [#317](https://github.com/mattmarshall/ratio/issues/317) / [#318](https://github.com/mattmarshall/ratio/issues/318), activation [#324](https://github.com/mattmarshall/ratio/issues/324)). Plaid Link creation/exchange binds expiring one-time state to the WorkOS subject/book; the local Personal runner AES-256-GCM seals the Item token, cursor, and pending index per membership, survives restart, retires pending rows when a posted transaction names them, and revokes before deleting custody. Transactions Sync parses numeric JSON as Decimal, requires an explicit rule choice, exposes pending rows without posting, refuses modified/removed rows, and keeps stable retry ids. first-party Connect apps call ConnectApiUrl; production credentials, Dashboard redirect registration, webhooks, and live institution evidence remain. Does not close #165 |
| `connect/tax-pack/` | First-party Connect app for household tax-pack export ([#166](https://github.com/mattmarshall/ratio/issues/166)). `ListDisposals` replays one selected view and cites the exact relieved lots, entry, sale prefix, observed journal cut, historical configuration and later wash write; the app maps those cites to 8949-ish CSV without recomputing basis. Each row uses its pinned holding-period threshold, so a later active configuration cannot reclassify it. Mixed acquired dates stay unclassified — `Ratio.Lots.PoolPeriod`, not an invented FIFO oldest date or two Form 8949 boxes. The deployment includes a deterministic `personal-tax-walkthrough` seed with a later wash replacement and a missing-date disposal. The dedicated WorkOS OAuth application is registered, and a live permitted-book export proved both rows with their exact cites. IRS e-file and a CPA portal are refused. |
| `connect/goals/` | First-party Connect app for Personal net-worth goals and what-if scenarios ([#168](https://github.com/mattmarshall/ratio/issues/168), book-scoped sheet [#355](https://github.com/mattmarshall/ratio/issues/355)). Its registered public-PKCE client verifies one permitted independent Personal book, reads that Book's period `sheet-*`, and retains the exact book, filter, configuration digest/revision, and account resource cites through live evaluation; no Fund is created or inferred, and an unposted sheet stays unset. Every `ListAccounts` row carries the active configuration digest and monotonic control revision after the server checks both before and after the fold, so even A → B → A promotion refuses; the app requires every row's digest to match the selected Book, and every row must match `chart_for(Personal)`. The legacy Fund-scoped account route remains compatible. Opt-in scenario journals use allowlisted Personal templates; closed-through and empty-allowlist refuse. First-party Connect apps call ConnectApiUrl. Not a cash forecast. Deployed independent-Book evidence remains on #168 and #355 |
...
| `connect/calendar-bills/` | First-party Connect app for Personal calendar bills sync ([#163](https://github.com/mattmarshall/ratio/issues/163), provider slice [#319](https://github.com/mattmarshall/ratio/issues/319), activation [#324](https://github.com/mattmarshall/ratio/issues/324)). A bounded Google Events sync uses the read-only events scope and one concrete calendar; Google expands recurring series into dated instances. The local Personal runner binds Google state to WorkOS subject/book, AES-256-GCM seals refresh/access tokens plus sync state per membership, refreshes access, retries a 410 as full sync with the retained id/etag index, and revokes before deletion. Only explicit private Ratio amount/currency/kind metadata reaches allowlisted `scheduled_income` / `scheduled_spend`; titles never classify. Exact id/etag retries are non-posting; changed/deleted imported occurrences refuse. Closed-through, empty-allowlist, payroll, and envelope refuse. WorkOS registration and live Google credentials/consent/calendar evidence remain. Does not close #163. Does not reopen #164. Does not redo #218 |
...
| `connect/lp-portal/` | First-party Connect app for an LP / investor portal ([#161](https://github.com/mattmarshall/ratio/issues/161)). Cites partner capital, commitments / undrawn, `CapitalNotice` (digest + pinned cut + posted amounts + trade date), GetBook `partner_cut` / `special_allocations` / `allocation_facts` / `fee_receivable` / trial balance, and NavStrike / period roll-forward already on the book (`partners:read`, `statements:read`, `nav:read`; optionally `books:read`). Missing cites stay unset — never a silent 1/N of book NAV, a callable-zero commitment, a NAV 0.00, or an empty-digest-as-success. `as_html` is a read-only cite-backed page in this tree. `html_portal` / `lp_directory` / `document_vault` / `payments_initiate` stay refuses. Read-only: no `journals:post`. No kernel HTML portal. No IRR / TVPI / waterfall. Drip elections stay leftover on #161 / #177. first-party Connect apps call ConnectApiUrl. This app's Connect grant is proven. #22 stays open for other Connect apps. Does not close #161. Leaves issue 22 open. Does not close #150. Does not close #177. Does not reopen #151 |
...
`//crates/ratio-console:recovery_test` cover a disposable local restore,
