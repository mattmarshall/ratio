...
| `connect/goals/` | First-party Connect app for Personal net-worth goals and what-if scenarios ([#168](https://github.com/mattmarshall/ratio/issues/168)). Its registered public-PKCE client verifies one permitted Personal book, reads a period `sheet-*`, and retains the exact book, filter, and account resource cites through live evaluation; an unposted sheet stays unset. Exact Book metadata must match before and after the fold, and every row must match `chart_for(Personal)`. The account fold does not carry a configuration digest, so the app does not mislabel the surrounding Book digest as its pin. Opt-in scenario journals use allowlisted Personal templates; closed-through and empty-allowlist refuse. First-party Connect apps call ConnectApiUrl. Not a cash forecast. Live evidence remains on #168 |
