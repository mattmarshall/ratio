...
| `connect/bank-feed/` | First-party Connect app for Personal bank feeds ([#165](https://github.com/mattmarshall/ratio/issues/165), provider slice [#317](https://github.com/mattmarshall/ratio/issues/317)). Mapper + allowlist + closed-through / conservation refusals; bounded Plaid Transactions Sync parses numeric JSON as Decimal, requires an explicit rule choice, exposes pending rows without posting, refuses modified/removed rows, keeps stable retry ids, and supports Item removal. first-party Connect apps call ConnectApiUrl; Plaid Link, credentials, token custody, webhooks, and live institution evidence remain. Does not close #165 |
