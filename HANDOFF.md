...
| `connect/goals/` | First-party Connect app for Personal net-worth goals and what-if scenarios ([#168](https://github.com/mattmarshall/ratio/issues/168)). Its registered public-PKCE client verifies one permitted Personal book and its fund association, reads that fund's period `sheet-*`, and retains the exact book, filter, configuration digest/revision, and account resource cites through live evaluation; an independent Book refuses because core has no book-scoped sheet, and an unposted sheet stays unset. Every `ListAccounts` row carries the active configuration digest and monotonic control revision after the server checks both before and after the fold, so even A → B → A promotion refuses; the app requires every row's digest to match the selected Book, and every row must match `chart_for(Personal)`. Opt-in scenario journals use allowlisted Personal templates; closed-through and empty-allowlist refuse. First-party Connect apps call ConnectApiUrl. Not a cash forecast. Live evidence remains on #168 |
