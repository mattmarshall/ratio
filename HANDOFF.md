...
| `connect/calendar-bills/` | First-party Connect app for Personal calendar bills sync ([#163](https://github.com/mattmarshall/ratio/issues/163), provider slice [#319](https://github.com/mattmarshall/ratio/issues/319)). A bounded Google Events sync uses the read-only events scope and one concrete calendar; Google expands recurring series into dated instances. Only explicit private Ratio amount/currency/kind metadata reaches allowlisted `scheduled_income` / `scheduled_spend`; titles never classify. Exact id/etag retries are non-posting; changed/deleted imported occurrences refuse, and expired sync tokens require full sync with the prior index retained. Closed-through, empty-allowlist, payroll, and envelope refuse. WorkOS registration and live Google credentials/consent/token custody/calendar evidence remain. Does not close #163. Does not reopen #164. Does not redo #218 |
